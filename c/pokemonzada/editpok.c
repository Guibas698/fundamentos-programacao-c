...
void pokemon_do_jogador(FILE* file, struct base_pokemon* pokemon)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        rewind(file);
        while (fscanf(file, "%d %s %s %d %d %d %d %d %d", &novo_pokemon.id, novo_pokemon.nome, novo_pokemon.elemento, &novo_pokemon.atk, &novo_pokemon.matk, &novo_pokemon.def, &novo_pokemon.mdef, &novo_pokemon.hp, &novo_pokemon.sp) == 9) {
...
void pokemon_da_maquina(FILE *file, struct base_pokemon *pokemon) {
    ...
    while (fscanf(file, "%d %s %s %d %d %d %d %d %d", &temp.id, temp.nome, temp.elemento, &temp.atk, &temp.matk, &temp.def, &temp.mdef, &temp.hp, &temp.sp) == 9) {
        total_pokemons++;
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j <= random_index; j++) {
            if (fscanf(file, "%d %s %s %d %d %d %d %d %d", &temp.id, temp.nome, temp.elemento, &temp.atk, &temp.matk, &temp.def, &temp.mdef, &temp.hp, &temp.sp) == 9) {
                if (j == random_index) {
...
int calcularDano(struct base_pokemon atacante, struct base_pokemon defensor, int golpe) {
    ...
    if (golpe == 1) { // Golpe físico (atk)
        ...
        if (atacante.elemento == 1 && defensor.elemento == 2) {
            dano = atacante.matk * 2 - defensor.mdef;
        } else if (atacante.elemento == 2 && defensor.elemento == 3) {
            dano = atacante.matk * 2 - defensor.mdef;
        } else if (atacante.elemento == 3 && defensor.elemento == 4) {
            dano = atacante.matk * 2 - defensor.mdef;
        } else if (atacante.elemento == 4 && defensor.elemento == 1) {
            dano = atacante.matk * 2 - defensor.mdef;
        } else if (atacante.elemento == 1 && defensor.elemento == 4) {
            dano = atacante.matk - defensor.mdef;
        } else if (atacante.elemento == 2 && defensor.elemento == 1) {
            dano = atacante.matk - defensor.mdef;
        } else if (atacante.elemento == 3 && defensor.elemento == 2) {
            dano = atacante.matk - defensor.mdef;
        } else if (atacante.elemento == 4 && defensor.elemento == 3) {
            dano = atacante.matk - defensor.mdef;
        }
    }
...
void batalha(FILE* file, struct base_pokemon* pokemons_jogador, struct base_pokemon* pokemons_bot) {
    int escolha;
    ...
    while (adicionando_pokemon) {
        ...
        switch (menu_selection) {
            ...
            case 4:
                ...
                // Luta começa agora
                for (int i = 0; i < 6; i++) {
                    batalha(file, &pokemons_jogador[i], &pokemons_bot[i]);
                }
                break;
            default:
